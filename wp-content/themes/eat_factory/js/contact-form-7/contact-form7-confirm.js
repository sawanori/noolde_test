...
jQuery(document).ready(function(){
	...
	var option = {
		pages : [
			{
				'path' : ['/contact/','/contact/franchise/','/contact/recruit/','/en_test/contact/','/contact/location/'],
				// 'path' : ['/contact/recruit/','/en_test/contact/','/contact/location/'],
				'button' : {
		...
		validates : {
			...
			email : {
				match : /^([\w!#$%&\'*+\-\/=?^`{|}~]+(\.[\w!#$%&\'*+\-\/=?^`{|}~]+)*|"([\w!#$%&\'*+\-\/=?^`{|}~. ()<>\[\]:;@,]|\\[\\"])+")@(([a-zA-Z\d\-]+\.)+[a-zA-Z]+|\[(\d{1,3}(\.\d{1,3}){3}|IPv6:[\da-fA-F]{0,4}(:[\da-fA-F]{0,4}){1,5}(:\d{1,3}(\.\d{1,3}){3}|(:[\da-fA-F]{0,4}){0,2}))\])$/,
				before : '',
				after : 'メールアドレスが正しい形式ではありません。'
			}
		}
	};
